...
Adaptive system-level evaluation with mode switching and cache updates.
"""
...
def main():
    ...
    for it in tqdm(selected_items, desc="System-Level Eval"):
        ...
        for snr in snr_list:
            ...
            # NOTE: if handshake utterance, you can still "attempt update" or just treat as already available.
            # Here we allow update attempt even for handshake, because in real system the prompt can be new.
            ...
            if snr >= args.snr_th_mode2:
                mode = 2
                update_success, tx_used, num_tbs, budget = simulate_harq_update(
                    snr_db=snr,
                ...
                status_tag = "Mode2_Updated" if update_success else "Mode2_Frozen"

            elif snr >= args.snr_th_mode1:
                mode = 1
                update_success, tx_used, num_tbs, budget = simulate_harq_update(
                    snr_db=snr,
                    payload_bytes=payload_bytes,
                ...
                status_tag = "Mode1_Updated" if update_success else "Mode1_Frozen"
            else:
                mode = 0
                update_success = False
                tx_used = 0
                status_tag = "Mode0_Frozen"

            # update cache if success
            if update_success:
                acoustic_frozen_library[spk_id] = tx_acoustic.copy()
                active_acoustic = tx_acoustic
            else:
